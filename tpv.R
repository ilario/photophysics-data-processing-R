...
files <- list.files(path=".", pattern="^TPV.*\\.txt.table$");
...
#	if(!file.exists(paste(x, "-biexp-log.png", sep="")) | !file.exists(paste(x, "-monoexp-log.png", sep=""))){
		...
		#workaround
		header = as.numeric(system(paste("head -6 '", paste(x,".txt",sep=""), "' | tail -3|sed 's/\r$//' | cut -f2 -d' '", sep=""), intern = TRUE))
		...
		write.table(outputDeltaV, file="outputDeltaV.txt", append=TRUE, col.names=F, row.names=F, quote=F);
		...
		capture.output(summary(fit), file=paste(x, "-fit", sep=""),  append=TRUE);
		C <- coef(fit)["C"];
		...
		step <- 0.5;
		removedPoints <- 0
		while(!file.exists(paste(x, "-biexp.png", sep="")) & removedPoints < thresholdBiexp){
	...
	tryCatch({
		...
		tryCatch({
			...
			graphics.off();
#			tryCatch({system("killall qiv")},error=function(e) print("ciao"));
#			system(paste("qiv ", x, "-biexp.png &", sep=""));
#			tryCatch({system("killall feh")},error=function(e) print("ciao"));
#			system(paste("feh -. ", x, "-biexp.png &", sep=""));
...
if(logy){			print("Biexp/Plot/Log: Performing");
		tryCatch({
			...
			png(paste(x, "-biexp-log.png", sep=""), width = 1280, height = 800);
...
if(logx){
		...
		tryCatch({
			...
			png(paste(x, "-biexp-logx.png", sep=""), width = 1280, height = 800);
...
#		output <- c(x, paste("y0=",signif(A2,5),"\\pm", signif(A2.err,2), sep="\t"),paste("A1=",signif(slowampl,5), "\\pm", signif(slowampl.err,2), sep="\t"),paste("t1=",signif(slowdecay,5), "\\pm", signif(slowdecay.err,2), sep="\t"),paste("A2=",signif(fastampl,5), "\\pm", signif(fastampl.err,2), sep="\t"),paste("t2=", signif(fastdecay,5), "\\pm", signif(fastdecay.err,2), sep="\t"),"");
		write.table(output, file="output-biexp.txt", append=TRUE, col.names=F, row.names=F, quote=F);
		
		biexpsuccess = 1;
		break
	}, error=function(e) print("Biexp/Fit: Error"));
		};
		if(file.exists(paste(x, "-biexp.png", sep=""))) {break}
		else {
...
if(logy){
		...
		png(paste(x, "-monoexp-log.png", sep=""), width = 1280, height = 800);
...
if(logx){
		...
		png(paste(x, "-monoexp-logx.png", sep=""), width = 1280, height = 800);
...
if(residuals){
		...
		graphics.off();
                ...
                write.table(outputmonoexp, file="output-monoexp.txt", append=TRUE, col.names=F, row.names=F, quote=F);

		tryCatch({
			capture.output(anova(fit,fit2), file=paste(x, "-fit", sep=""),  append=TRUE);
...
if(powerlaw){
		...
		capture.output(summary(fit3), file=paste(x, "-fit", sep=""),  append=TRUE);
		K <- H/G;
		K.err <- K*(H.err/H + G.err/G)

		print("PowerMonoexp/Plot/Linear: Performing");
		tryCatch({
			png(paste(x, "-powermonoexp.png", sep=""), width = 1280, height = 800);
...
if(logy){
		...
		tryCatch({
			png(paste(x, "-powermonoexp-log.png", sep=""), width = 1280, height = 800);
...
if(robust){
		...
		capture.output(summary(fitR), file=paste(x, "-fit", sep=""),  append=TRUE);
#		AR <- coef(fitR)[".lin1"]; BR <- coef(fitR)[".lin2"]; 
		...
		png(paste(x, "-robustmonoexp.png", sep=""), width = 1280, height = 800);
...
if(logy){
		...
		png(paste(x, "-robustmonoexp-log.png", sep=""), width = 1280, height = 800);
...
if(logx){
		...
		png(paste(x, "-robustmonoexp-logx.png", sep=""), width = 1280, height = 800);
...
if(residuals){
		...
		graphics.off();
                ...
                write.table(outputmonoexp, file="output-robustmonoexp.txt", append=TRUE, col.names=F, row.names=F, quote=F);

		tryCatch({
			capture.output(anova(fit,fitR), file=paste(x, "-fit", sep=""),  append=TRUE);
		}, error=function(e) print("Anova comparison: Error"));
		break;
...
if(powerlaw){if(robust){
		...
		capture.output(summary(fit3r), file=paste(x, "-fit", sep=""),  append=TRUE);
		Kr <- I/L;
		Kr.err <- Kr*(I.err/I + L.err/L)

		print("RobustPowerMonoexp/Plot/Linear: Performing");
		tryCatch({
			png(paste(x, "-robustpowermonoexp.png", sep=""), width = 1280, height = 800);
...
if(logy){
		...
		tryCatch({
			png(paste(x, "-robustpowermonoexp-log.png", sep=""), width = 1280, height = 800);
...
if(robust){
		...
		tries=0
		while(!file.exists(paste(x, "-robustbiexp.png", sep="")) & tries < thresholdRobustBiexp){
		tries=tries+1
#		for(rand in seq(1, 10, by=0.5)){
	tryCatch({
#		C2R <- slowdecay#*(2^runif(1,-rand,rand))*10;
#		F2R <- fastdecay#*(2^runif(1,-rand,rand))/10;
		fit2R <- nlrob(voltage ~ A2R + C2Rampl*exp(-time/C2R) + F2Rampl*exp(-time/F2R), start=list(A2R=A2, C2Rampl=slowampl, C2R=slowdecay, F2Rampl=fastampl, F2R=fastdecay),trace=F,data=temp);
			...
			if(coef(fit2R)["C2Rampl"] < 0 | coef(fit2R)["F2Rampl"] < 0){print("RobustBiexp/Fit: Bad result");}# next};
			#print(paste(startingvoltage, "=>", coef(fit2)[".lin1"], "/// ... =>", coef(fit2)[".lin2"], "///", Crand, "=>", coef(fit2)["C"], "/// ... =>", coef(fit2)[".lin3"], "--", Frand, "=>", coef(fit2)["F"]));
#			if(coef(fit2)["C"] > coef(fit2)["F"]) {slowampl <- coef(fit2)[".lin2"]; slowdecay <- coef(fit2)["C"]; fastampl <- coef(fit2)[".lin3"]; fastdecay <- coef(fit2)["F"]; slowampl.err <- summary(fit2)$coefficients[".lin2",2]; slowdecay.err <- summary(fit2)$coefficients["C",2]; fastampl.err <- summary(fit2)$coefficients[".lin3",2]; fastdecay.err <- summary(fit2)$coefficients["F",2]} else {fastampl <- coef(fit2)[".lin2"]; fastdecay <- coef(fit2)["C"]; slowampl <- coef(fit2)[".lin3"]; slowdecay <- coef(fit2)["F"]; fastampl.err <- summary(fit2)$coefficients[".lin2",2]; fastdecay.err <- summary(fit2)$coefficients["C",2]; slowampl.err <- summary(fit2)$coefficients[".lin3",2]; slowdecay.err <- summary(fit2)$coefficients["F",2]};
			...
			capture.output(summary(fit2R), file=paste(x, "-fit", sep=""),  append=TRUE);

			print("RobustBiexp/Plot/Linear: Performing");
		tryCatch({
			png(paste(x, "-robustbiexp.png", sep=""), width = 1280, height = 800);
...
if(logy){			print("RobustBiexp/Plot/Log: Performing");
		tryCatch({
			...
			png(paste(x, "-robustbiexp-log.png", sep=""), width = 1280, height = 800);
...
if(logx){
		...
		tryCatch({
			png(paste(x, "-robustbiexp-logx.png", sep=""), width = 1280, height = 800);
...
#		output <- c(x, paste("y0=",signif(A2,5),"\\pm", signif(A2.err,2), sep="\t"),paste("A1=",signif(slowampl,5), "\\pm", signif(slowampl.err,2), sep="\t"),paste("t1=",signif(slowdecay,5), "\\pm", signif(slowdecay.err,2), sep="\t"),paste("A2=",signif(fastampl,5), "\\pm", signif(fastampl.err,2), sep="\t"),paste("t2=", signif(fastdecay,5), "\\pm", signif(fastdecay.err,2), sep="\t"),"");
		write.table(outputrobust, file="output-robustbiexp.txt", append=TRUE, col.names=F, row.names=F, quote=F);
		
		robustbiexpsuccess = 1;
		break
	}, error=function(e) print("RobustBiexp/Fit: Error"));
		#};
		if(file.exists(paste(x, "-robustbiexp.png", sep=""))) {break}
