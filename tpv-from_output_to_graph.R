...
tpvFromOutputToGraph <- function(tpvdir="tpv")
{
  ...
  if(file.exists(file.path(tpvdir, "output-mixedbimono.txt")))
  {
    ...
    #errbar(tpv$Voc, tpv$T1, tpv$T1+tpv$T1.error, tpv$T1-tpv$T1.error,log="y",add=TRUE, pch="")
    #errbar(tpv$Voc, tpv$T2, tpv$T2+tpv$T2.error, tpv$T2-tpv$T2.error,log="y",add=TRUE, pch="")
    points(tpv$Voc, tpv$T1, pch=20, col="blue")#, cex=4*tpv$A1/(tpv$A1+tpv$A2));
    points(tpv$Voc, tpv$T2, pch=1, col="blue")#, cex=4*tpv$A2/(tpv$A1+tpv$A2));
    
    par(new = TRUE)
    plot(1, ylim=c(min(tpv$A1,tpv$A2[!is.na(tpv$A2)]),max(tpv$A2[!is.na(tpv$A2)],tpv$A1)), xlim=c(min(tpv$Voc),max(tpv$Voc)), axes = FALSE, bty = "n", xlab = "", ylab = "")
